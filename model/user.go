...
type User struct {
	...
	ProviderLockedQuota int            `json:"provider_locked_quota" gorm:"type:int;default:0;column:provider_locked_quota"` // provider endpoint deposits locked from spendable quota
...
func decreaseUserQuota(id int, quota int) (err error) {
	result := DB.Model(&User{}).Where("id = ? AND quota - provider_locked_quota >= ?", id, quota).Update("quota", gorm.Expr("quota - ?", quota))
	if result.Error != nil {
		return result.Error
	}
